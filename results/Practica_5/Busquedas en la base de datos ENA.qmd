---
title: "Practica 5: Búsquedas programáticas en la base de datos ENA"
lang: es
date: today
execute: 
  output: true
...
mucha información sobre cada uno de los *registros*. Desde la línea de comandos o desde un lenguaje de programación cualquiera podemos descargar tanto **metadatos**, mediante la API del *portal* de ENA; como **registros** (es decir, secuencias y la información que las acompaña), mediante la API del *navegador* (*browser*) de ENA.
...
curl_download('https://www.ebi.ac.uk/ena/portal/api/search?result=assembly&query=tax_tree(6157) AND genome_representation="full"&fields=version,tax_id,scientific_name,last_updated,base_count&limit=0',
              destfile = './data/Practica_05/z1.txt')

#Este bloque no funciona y me daba error porque no reconocía que la URL estaba
#bien escrita. Para ello, sería sido necesario añadir el prefijo URLencode.
...
#Esta no me ha dado ningún problema
```
...
curl_download(URLencode(URL), destfile = './data/Practica_05/z2.txt')
#Esta tampoco ha dado ningún problema
```
:::
...
:::{.callout-note icon=false}
## Solución
Para poder elaborar dicha URL; lo primero que queremos es identificar el ID de *Drosophila*. Para ello, nos metemos en el NCBI taxonomy browser y encontramos que el ID es 7215. Por otro lado, tenemos que tener en cuenta que estamos buscando exclusivamente la enzima alcohol deshidorgenasa, es decir, una secuencia de una proteína. Por tanto, nuestra search será de una secuencia. 

Además, como solo queremos las alcohol deshidorgenasas, vamos a ponerle otra query adicional al ID, que será que aparezca el nombre alcohol deshidrogenasa. 
Por último, tenemos que decirle que columnas queremos que nos aparezcan. Teniendo en cuenta las preguntas, tienen que ser las correspondientes a la longitud(`base_count`), al nombre científico(`scientific_name`) y a la fecha de publicación(`first_public`). *Para conocer los nombres de las tablas, primero pondré `fields=all`.*
...
curl_download(
  URLencode('https://www.ebi.ac.uk/ena/portal/api/search?result=sequence&query=tax_tree(7215) AND description="alcohol dehydrogenase"&fields=scientific_name,first_public,base_count&limit=0'), destfile = './results/Practica_5/drosophila.txt')

```

Esto te crea un archivo .txt dónde puedes buscar todas las respuestas.
...
científico. Para ello, debemos separar las partes de cada nombre delimitadas por un espacio en blanco, con la función `strsplit()`. Y por último usamos la función `sapply()` para aplicar a cada nombre científico la función de selección de componentes, `[`, y la opción "1", para seleccionar sólo el primero de los componentes en que habíamos dividido cada nombre científico.
...
:::{.callout-note icon=false}
## Solución 3
Vamos a ir por partes para solucionar este ejercicio. Lo primero que vamos a hacer es ordenar la tabla en función de las fechas:

```{r}
platelmintos <- platelmintos[order(platelmintos$last_updated), ]
```

Una vez ordenados, vamos a crear una nueva columna dentro de nuestra tabla de platelmintos que nos represente la longitud acumulada. Para ello, usaremos la función cumsum, que nos da un vector con la suma acumulada de un conjunto de valores:

```{r}
platelmintos$acumulado <- cumsum(platelmintos$base_count)
```

Por último, volvemos a representar la gráfica:

```{r}
#| fig.width: 10
par(mar = c(8,7,1,1))
boxplot(acumulado ~ last_updated, data = platelmintos,
        las = 2, xlab = '', ylab = '', cex.axis = 0.8)
mtext('Fecha', side = 1, line = 7)
mtext('Longituda acumulada', side = 2, line = 5)
```
:::
