---
title: "Practica 2: Control de versiones con Git"
lang: es
date: today
execute: 
  eval: true
...
# Preparación
Para esta práctica necesitamos tener Git instalado. Podéis comprobar si ya está instalado desde el terminal de Bash, tecleando `git --version`. En WSL2, podéis instalar Git desde el terminal con la orden `apt install git`. En MacOS, hay [varias opciones](https://git-scm.com/install/mac), pero la más sencilla parece ser instalar las *command line tools*, también desde un terminal, mediante la orden `xcode-select --install`.
...
# Configuración
Para facilitar la comunicación, es importante usar el correo electrónico institucional y el nombre verdadero. Utilizad el mismo correo electrónico al crear una cuenta en Github.

```{bash}
git config --global user.name "DjTheorn"
git config --global user.email "tecas@alumni.uv.es"
```
...
::: {.callout-tip title="Problemas cuando estás usando WSL"}
Si estamos usando WSL y R en la interfaz de Windows, es posible que tengas ciertos problemas para utilizar bash, entre ellos que no te detecte el comando git. Para ello, tienes que añadir delante de git el comando `wsl`, de forma que quedaría algo como:
```
wsl git config --global core.editor "nano -W"
```
De esta forma, le estás diciendo a tu ordenador dónde tiene que buscar el comando git y te dejará de dar errores. Igualmente, lo más recomendable es usar RStudio server de forma online.
:::
...
Desde la carpeta principal, es decir, PBioinformatica en mi caso, en el terminal ejecutamos `git init`. Se habrá creado una carpeta oculta, `.git`. A partir de ese momento, Git observará las modificaciones de todos los archivos en esa carpeta y **en todas sus subcarpetas**.
...
Utiliza `git add ...` para añadir archivos al área de preparación o "escenario". Si usamos `git add ..`; se añadirán todos los cambios de carpetas y subcarpetas sin necesidad de ir una por una añadiendolas todas. Consulta `git status` y cuando los hayas añadido todos, ejecuta `git commit -m "...", donde debes sustituir los "..." por un mensaje entrecomillado que describa los cambios introducidos en la carpeta. Por ejemplo:
...
Consulta `git status` y también `git log`. Asegúrate de que la rama lleva el nombre que habías elegido.
...
Para ignorar ciertos archivos, podemos crear el archivo `.gitignore`, directamente en la carpeta de trabajo. Es un archivo de texto, y su contenido es una lista de archivos a ignorar. En mi caso, como quería que en git apareciera la carpeta data para tener un pelín más ordenado todo, he modificado un poco el gitignore, que aparece tal que:
...
2. Ignora el contenido de la carpeta local `data`, excepto el archivo `.gitkeep` en la propia carpeta y en todas sus subcarpetas
3. Ignora la carpeta `.RData` y `.Rhistory`, recomendación de gemini. El archivo `.RData` guarda información sobre archivos que se encuentra en mi memoria del ordenador pero que en muchos casos no están en git, siendo efectviamente inútiles. Por otro lado, `.Rhistory` guarda absolutamente todo lo que escribo en la consola, por lo que si en cualquier momento escribí algo de información confidencial, una contraseña, o algo similar, podría ser arriesgado ponerlo a la vista de todo el mundo. Este cambio fue impuesto al hacer esta práctica, por lo que no ha afectado a la práctica 1 que si tiene el `..Rhistory`
4. Ignora la carpeta `.Rproj.user`, que suele dar problemas cuando otra persona descarga tu git, además de que ensucia el regstro de commits.
   
# Crear un repositorio en Github
Tienes que haber creado una cuenta de usuario o ususaria. Github te guía en la
creación de un nuevo repositorio. Es recomendable darle el mismo nombre que tiene la carpeta en la que has creado tu repositorio local. Puedes configurar el repositorio en Github como privado, si prefieres que nadie lo vea. Pero en ese caso, asegúrate de añadir como colaborador a tu profesor de Bioinformática.

Github te da la opción de añadir un archivo `README.md`, un archivo `.gitignore` y una licencia. Si ya tienes un README.md local, no hace falta crear otro. En cuanto a la licencia, los guiones de prácticas están ya publicados con licencia
[Creative Commons Attribution Share Alike 4.0 International](https://creativecommons.org/licenses/by-sa/4.0/deed.en).
Si usas estos guiones como punto de partida, deberías respetar la obligación de compartir el trabajo derivado con el mismo tipo de licencia. Como esta licencia no aparece entre las opciones de Github, lo correcto sería añadir un archivo de texto llamado
...
a la rama "Base" del remoto "origin", y que además establezca esa rama remota
...
# Comandos importantes para recordar en relación a git:
| Comando | Función |
| ------- | ------- |
| git status | te dice el estado de tu git local: en que branch te encuentras, que cambios no tienen commit, que archivos no han sido añadidos, etc. |
| git add "a" | añade el archivo "a" a la zona de preparación. |
| git add .. | añade todos los archivos nuevos o con modificaciones a la zona de preparación. |
| git commit -m "a"| añade el menasje "a" a todos los cambios producidos con el último git add. |
| git rm --cached "a" | elimina el archivo "a" de github, pero no de tu ordenador. Si el archivo está dentor de una carpeta, cambia a "carpeta/a". |
| git push | modifica github con los cambios ejecutados en tu repositorio local. Tiene que haberse producido un git add y un git commit con anterioridad. |
| git pull | modifica el repositiorio local con los archivos subidos en github.
