...
`dplyr`, `kableExtra` y `jsonlite`. Para descargar algunos datos de ejemplo, usaremos los programas `datasets` y `dataformat` [@OLeary2024], que pueden descargarse en [este enlace](https://www.ncbi.nlm.nih.gov/datasets/docs/v2/command-line-tools/download-and-install/).
...
# Datos
Supón que tenemos interés en descargar un genoma de referencia de cualquier especie del género *Trypanosoma*, así como su anotación funcional, en formato
...
if [ ! -e  ./data/Practica_04/trypanosoma_annotated.txt ]; then
   ./data/Practica_04/datasets summary genome taxon Trypanosoma --annotated --as-json-lines | \
   ./data/Practica_04/dataformat tsv genome --fields \
      accession,organism-name,assminfo-release-date,assminfo-level,assmstats-atgc-count,annotinfo-featcount-gene-total,assminfo-type,assminfo-status > ./data/Practica_04/trypanosoma_annotated.txt
fi
```
...
library('kableExtra')
genomas <- read.table('./data/Practica_04/trypanosoma_annotated.txt',
                      header = TRUE, sep = '\t')
...
:::{.callout-note icon=false}
## Respuestas
1. La única diferencia visible es el nombre (uno comienza por GCA y el otro por GCF), pero esta diferencia nos indica que la calidad de ambos archivos es distinta. Los genomas marcados como GCA son archivos que han sido subidos a la base de datos tal cual lo subieron los investigadores que lo secuenciaron; de forma que no han pasado por un proceso de validación, procesado y estandarización por ningún organismo de calidad. Por otro lado, los genomas marcados por GCF son genomas que sí han pasado por este proceso de estandarización por el equipo de RefSeq del NCBI, garantizando una calidad correcta.

2. Si tan solo queremos identificar las lineas repetidas, R tiene una función que se llama `duplicated`:
```{r}
esta_duplicado <- duplicated(genomas)
print(esta_duplicado)
```

Esto sirve para líneas exactamente idénticas; pero de forma habitual no existen estas líneas porque tienen un identificador diferente. Por tanto, vamos a buscar lineas idénticas ignorando el identificador: 

```{r}
esta_duplicado_sinID <- duplicated(genomas[, -1])
print(esta_duplicado_sinID)
```

Y si no solo queremos saber cuales están duplicadas si no que nos interesa que nos diga toda la información de las que están duplicadas:

```{r}
datos_duplicados_sinID <- genomas[esta_duplicado_sinID,]
print(datos_duplicados_sinID)
```

Solo pone los GCA porque en la tabla siempre aparecen segundos; pero están copiados de un GCF correspondiente.

Gemini me ha mencionado también un paquete llamado `janitor` que al parecer funciona muy bien, usando la función ````get_dupes(genomas,2)```.
:::

# Ordenar una tabla
La función `order()` produce los índices ordenados de los elementos de un vector.

Consulta su ayuda y experimenta con ella. Vamos a usar esta función para ordenar las filas del marco de datos `genomas` por fecha, que es la tercera columna, llamada `Assembly.Release.Date`.
...
Casualmente, la fecha estaba en formato "%Y-%m-%d", en el que el orden alfabético coincide con el orden temporal. Al crear `genomas`, `read.table()` había interpretado la tercera columna como texto. Para que las fechas sean entendidas como tales, podemos usar la función `as.Date()`:
...
if [ ! -e ./data/Practica_04/ncbi_dataset/data/GCA_022059095.1/genomic.gff ]; then
   if [ ! -e ./data/Practica_04/ncbi_dataset.zip ]; then
      ./data/17-02-2026/datasets download genome accession GCA_022059095.1 --include genome,gff3 --filename ./data/Practica_04/ncbi_dataset.zip
   fi
   unzip ./data/Practica_04/ncbi_dataset.zip -d ./data/Practica_04/
fi
```
...
El formato "jsonlines" es una versión de JSON en la que cada registro ocupa una sola línea. Esto dificulta la visualización, pero agiliza el análisis de los datos en la línea de órdenes. Para ver mejor el contenido del archivo con la extensión `jsonl`, podríamos utilizar el programa [`jq`](https://jqlang.org/), o bien utilizar la función `prettify()` del paquete `jsonlite` de R:
...
library('jsonlite')
prettify(readLines('./data/Practica_04/ncbi_dataset/data/assembly_data_report.jsonl'))
```

Volviendo a lo que nos interesa, vamos a cargar en un marco de datos de una
sesión de R la anotación funcional del genoma escogido. Aunque existen funciones específicas para leer archivos GFF, usaremos la función básica `read.table()`, con alguna precaución.
...
gff <- read.table('./data/Practica_04/ncbi_dataset/data/GCA_022059095.1/genomic.gff', sep = '\t',
                  col.names = c('seqid', 'source', 'type', 'start', 'end',
                                'score', 'strand', 'phase', 'attributes'),
                  quote = '')
```

Al leer el archivo GFF con `read.table()`, debemos especificar que el separador de columnas es el tabulador y que no hay texto entrecomillado en el archivo.

Esto último se consigue con la opción `quote = ''`, y sirve para evitar que todo lo que se encuentre entre dos apóstrofes (*'*) se considere parte de un mismo campo de información, aunque sean cientos de líneas.
...
el principio (`head(gff)`) y el final (`tail(gff)`) del objeto, que es demasiado grande para mostrarlo en pantalla completo.
...
:::{.callout-note icon=false}
## Desafío
Identifica en cada uno de estos marcos de datos qué columnas son invariables. Esdecir, las columnas que tienen el mismo valor en todas las líneas. Y elimina esas columnas.
:::

:::{.callout-note icon=false}
## Solución
Para identificar que columnas son invariables, existen varios métodos, pero voy a usar uno que utiliza el paquete `dplyr`. Este paquete tiene una funnción de limpieza de tablas con un único comando:
```{r}
#| output: false
library(dplyr)
...
```
Lo repetimos con el resto de las tablas:
```{r}
mrna_limpio <- mrna %>%
  select(where(~ n_distinct(.) > 1))
...
Hay que tener cuidado pprque esto puede eliminar columnas que si que nos van a servir en análisis posteriores. Por ejemplo, al hacer esta limpieza del tipo "region", eliminamos la columna de start, puesto que siempre comienza por 1. Esto impide elaborar un resumen correcto como se nos pide en apartados futuros.
...
:::{.callout-note icon=false}
## Desafío
¿Sabrías completar el marco de datos `resumen` con los datos de los otros cuatro tipos de anotación funcional?
:::

:::{.callout-note icon=false}
## Solución
El paquete `dplyr` también tiene una función para añadir filas de forma muy simple. La idea es crear nuevos datasets con los datos de todas nuestras anotaciones y después juntarlos todos en el resumen. Para ello, hacemos:
```{r}
mrna$longitud <- mrna$end - mrna$start + 1
mrna_resumen <- data.frame(
   tipo       = 'mRNA',
   número     = nrow(mrna),
...
resumen <- bind_rows(resumen, mrna_resumen, exon_resumen, gene_resumen, region_resumen)

```
:::

Este tipo de operación es bastante habitual, y se puede hacer sin necesidad de
crear marcos de datos parciales como he hecho yo. La función `split()` divide las líneas de un marco de datos en tantos marcos de datos como valores diferentes (*niveles*) haya en el *factor* que usemos para definir los grupos. El resultado es un solo objeto: una *lista* de marcos de datos:
...
se pueden considerar un dialecto de R. Las funciones (o *verbos*) más interesantes de `dplyr` son `group_by()` y `summarise()`.
...
:::{.callout-note icon=false}
## Solución
Para poder hacer esto, lo primero que tenemos que saber es como han marcado en nuestro dataset que un gen pertenece a un cromosoma o a otro. Lo más habitual es que lo hayan hecho identificandoles diferentes id, lo que nos permite filtrar por seqid para saber cuantos genes pertenecen a cada uno:

```{r}
filtrado_genes <- gff %>% group_by(seqid) %>% summarise(
  cantidad_genes = sum(type == "gene"),
  genes_pos = sum(type == "gene" & strand == "+"),
  genes_neg = sum(type == "gene" & strand == "-")
)
```
...
:::{.callout-note icon=false}
## Solución
Lo vamos a hacer de forma similar a como lo hemos hecho antes:
```{r}
interesantes_genes <- interesantes[interesantes$type == 'gene', -3]
...
([bedtools](https://github.com/arq5x/bedtools2)), continuaremos practicando con marcos de datos de R. Ahora ya no podemos usar `%in%`, porque de cada cromosoma queremos unos segmentos diferentes. Una estrategia es crear cinco filtros booleanos, y después combinarlos con la operación lógica "o", es decir `|`. En cada filtro seleccionaremos las anotaciones que estén completamente contenidas en el intervalo indicado.
...
:::{.callout-note icon=false}
## Solución
El resultado quedaría tal que:
```{r}
f1 <- gff$seqid == 'JAJSEB010000048.1' & ! (gff$start <= 184638 & gff$end <= 184638) & ! (gff$start >= 253478 & gff$end >= 253478) 
f2 <- gff$seqid == 'JAJSEB010000012.1' & ! (gff$start <= 133444 & gff$end <= 133444) & ! (gff$start >= 348860 & gff$end >= 348860)
f3 <- gff$seqid == 'JAJSEB010000020.1' & ! (gff$start <= 73393 & gff$end <= 73393) & ! (gff$start >= 149614  & gff$end >= 149614)
f4 <- gff$seqid == 'JAJSEB010000016.1' & ! (gff$start <= 161247 & gff$end <= 161247) & ! (gff$start >= 272009 & gff$end >= 272009)
f5 <- gff$seqid == 'JAJSEB010000033.1' & ! (gff$start <= 123135 & gff$end <= 123135) & ! (gff$start >= 170687 & gff$end >= 170687)
...
:::{.callout-note icon=false}
En un archivo GFF, la llave será la columna *attributes*, dónde se indica un ID específico para cada dato además de información adicional.
:::
